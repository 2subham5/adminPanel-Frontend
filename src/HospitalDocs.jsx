...
<CardContent className="hospital-card-content">
  ...
  <List>
  {/* Map over listOfDoctors */}
  {hospital.listOfDoctors.map((doctor, index) => (
    <ListItem key={index}>
      <ListItemText
      ...
      <Button
        ...
        variant="outlined"
        style={{ color: '#008080' }}
        onClick={() => navigate(`/hospitalDoc/${hospital.hospitalId}`)}
      >
        Edit
      </Button>
      <Button
        ...
        onClick={() => navigate(`/hospitalDoc/add/${hospital.hospitalId}`)}
      >
        Add
      </Button>
      </CardActions>
